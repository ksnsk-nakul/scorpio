  ...
  <div v-if="isOpen && ragAvailable" class="fixed bottom-0 right-6 z-50">
    ...
    <div v-else class="h-[28rem] w-96 bg-white border border-slate-200 rounded-t-xl shadow-2xl flex flex-col overflow-hidden">
      <div class="h-11 px-4 flex items-center justify-between bg-slate-900 flex-shrink-0">
        <div class="flex items-center gap-1 flex-shrink-0">
          <!-- "Back to inbox" action, leftmost like LinkedIn's back arrow. -->
          <button @click="openThreadList" aria-label="Conversations" title="Conversations" class="p-1 text-slate-300 hover:text-white transition-colors">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
        </div>
        <span class="text-sm font-medium text-white truncate flex-1 text-center">{{ view === 'list' ? 'Conversations' : 'Advanced Search' }}</span>
        <div class="flex items-center gap-1 flex-shrink-0">
          <button @click="startNewChat" aria-label="New chat" title="New chat" class="p-1 text-slate-300 hover:text-white transition-colors">
      ...
      <template v-else-if="view === 'list'">
        <div class="flex-1 overflow-y-auto">
          <div v-if="!threadListLoaded" class="flex items-center gap-2 text-xs text-slate-400 px-4 py-10 justify-center">
            <span class="inline-block w-3 h-3 rounded-full border-2 border-slate-300 border-t-blue-500 animate-spin"></span>
            Loading…
          </div>
          <div v-else-if="threadList.length === 0" class="text-xs text-slate-400 text-center py-10">
          ...
          <div
            ...
            class="hover:bg-slate-50 cursor-pointer px-3 py-2.5 border-b border-slate-100 flex items-start justify-between gap-2"
          >
            <div class="min-w-0 flex-1">
              <p class="font-medium text-sm text-slate-800 truncate">{{ t.title }}</p>
              <p class="text-xs text-slate-500 truncate">
                <span v-if="t.last_message_role === 'user'">You: </span>{{ t.last_message }}
              </p>
            </div>
            <span class="text-xs text-slate-400 flex-shrink-0 whitespace-nowrap">{{ timeAgo(t.updated_at) }}</span>
          </div>
        </div>
      </template>
    </div>
...
const {
  isOpen, isMinimized, messages, loading, error, view, threadList, threadListLoaded,
  minimize, expand, newChat, deleteConversation, ask, loadHistory,
  openThreadList, openThread,
} = useAdvancedSearch()
const { confirmAction } = useConfirm()

// New chat while in the inbox view both starts a fresh conversation and
// switches back to the chat view so the empty conversation is visible.
const startNewChat = () => {
  newChat()
  view.value = 'chat'
}
