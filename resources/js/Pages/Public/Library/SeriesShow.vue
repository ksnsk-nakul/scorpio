<template>
  <component v-if="activeTemplateComponent" :is="activeTemplateComponent" :series="series" :books="books" />
  ...
  <div class="min-h-screen bg-white font-sans">
    <nav class="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-white/80 border-b border-slate-100">
      <div class="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        <span class="text-sm font-semibold text-slate-800">{{ series.name }}</span>
      </div>
    </nav>

    <main class="pt-20 pb-16 max-w-6xl mx-auto px-6">
      <h1 class="text-2xl font-bold text-slate-800 mb-2">{{ series.name }}</h1>
      <p v-if="series.description" class="text-sm text-slate-500 mb-8 max-w-2xl">{{ series.description }}</p>
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        <Link v-for="book in books.data" :key="book.slug" :href="`/library/books/${book.slug}`" class="group">
          ...
          <p class="text-xs text-slate-400 mt-0.5">Vol. {{ book.volume_number }}</p>
        </Link>
      </div>
...
const activeTemplateComponent = computed(() => resolvePublicPage(publicTemplate.value, 'SeriesShow'))
</script>
