    ...
    <main class="pt-14">
      ...
      <template v-if="page" v-for="block in (page.blocks ?? [])" :key="block.order">
        ...
        <section v-if="block.type === 'hero'" id="home"
          ...
          <div class="flex-1">
            <p class="text-orange-500 font-semibold text-xs uppercase tracking-[0.3em] mb-4">Hello, I'm</p>
          ...
          <div class="flex-1 hidden md:flex justify-center items-center">
            <div class="relative">
              <div class="absolute inset-0 rounded-3xl bg-gradient-to-br from-orange-100 to-blue-100 scale-110 opacity-60"></div>
              <img :src="block.data.image || '/images/profile.png'" :alt="owner.name"
                   class="relative w-[340px] h-[340px] object-cover drop-shadow-2xl rounded-3xl"
...
// ── Rotating hero text ───────────────────────────────────────────────────────
// Same setInterval-driven index-cycling as the original, but the phrase swap
// itself is now an anime.js fade-out/fade-in crossfade instead of Vue's
