<template>
  ...
  <div class="min-h-screen bg-white font-sans">
    ...
    <main class="pt-14">
      <section class="bg-slate-950 text-white py-20 px-6">
        <div class="max-w-4xl mx-auto">
          ...
          <p v-if="project.description" class="text-lg text-slate-300 leading-relaxed max-w-2xl mb-8">{{ project.description }}</p>
    ...
    <Teleport to="body">
      <div v-if="lightboxIndex !== null" class="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" @click.self="closeLightbox" @keydown.escape="closeLightbox">
        <button @click="closeLightbox" class="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-white text-xl flex items-center justify-center transition-colors">✕</button>
        <button v-if="media.length > 1" @click="prevItem" class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors">
        ...
        <button v-if="media.length > 1" @click="nextItem" class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        </button>
        <div v-if="media.length > 1" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
          <button v-for="(_, i) in media" :key="i" @click="lightboxIndex = i" class="w-2 h-2 rounded-full transition-colors" :class="i === lightboxIndex ? 'bg-white' : 'bg-white/30'" />
        </div>
      </div>
    </Teleport>
  </div>
